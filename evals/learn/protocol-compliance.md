...
The scoring rules that turn an eval transcript into a PASS or FAIL verdict.

> [!NOTE]
> **Management summary.** This rubric defines how to score an eval transcript: the per-case PASS/FAIL logic, the transcript evidence a verdict requires, the judgement calls that recur, and the aggregate conformance bar — all six cases passing on at least two of the three tested runtimes. Read it together with [the eval runbook](README.md), which tells you how to run a case in the first place. Version: ÆON Learn 0.3.0.
...
- **Simulation caveats** — anything you declared rather than genuinely constrained, for example "runtime actually has web access; unavailability was declared at session start". A caveat does not weaken a verdict, but omitting it does.
...
- **Evaluator error voids the run, not the case.** If you broke the simulation by hinting at expectations or answering out of profile, rerun the case in a fresh session rather than scoring the tainted transcript.
...
The six cases and what each one tests are listed in [the case index](cases/README.md).
