...
Slots are semantic, not typographic: a renderer may merge adjacent slots into flowing prose (`renderers/podcast.md`), but every slot's content MUST be identifiable in the canonical lesson.
...
**LEARN-S-9 (Retrieval)** — The retrieval slot MUST require an attempted recall of an earlier concept before the answer is shown, per `adaptation.md`. Re-summarising earlier sessions for the learner is not retrieval.
...
**LEARN-S-12** — Session length MUST respect the daily time budget agreed in the learning contract (`curriculum.md`). Formats are derived per `renderers/README.md` and offered only within the verified capability profile.

**LEARN-S-13** — After delivering a session, the agent SHOULD collect the lightweight adaptation signals of `adaptation.md` and MUST update journey progress in learner state (`../../protocol/state.md`).
